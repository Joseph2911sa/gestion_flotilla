...
<div class="card card-outline card-secondary mb-3">
    <div class="card-body py-2">
        <form action="{{ route('operador.mantenimientos') }}" method="GET"
            ...
            @if($vehiculoFiltro || $statusFiltro)
                <a href="{{ route('operador.mantenimientos') }}" class="btn btn-outline-secondary btn-sm">
        ...
        <div class="card-footer clearfix">
            <ul class="pagination pagination-sm m-0 float-right">
                @if($paginado['current_page'] > 1)
                    <li class="page-item">
                        <a class="page-link"
                           href="{{ route('operador.mantenimientos') }}?page={{ $paginado['current_page']-1 }}&vehicle_id={{ $vehiculoFiltro }}&status={{ $statusFiltro }}">«</a>
                    </li>
                @endif
                @for($p = 1; $p <= $paginado['last_page']; $p++)
                    <li class="page-item {{ $p === $paginado['current_page'] ? 'active' : '' }}">
                        <a class="page-link"
                           href="{{ route('operador.mantenimientos') }}?page={{ $p }}&vehicle_id={{ $vehiculoFiltro }}&status={{ $statusFiltro }}">{{ $p }}</a>
                    </li>
                @endfor
...
$(function () {
    $(document).on('click', '.btn-cerrar', function () {
        ...
        $('#formCerrar').attr('action', '/operador/mantenimientos/' + id + '/cerrar');
        $('#cost_cerrar').val('');
